...
let clickPowerDisplay = null;
let critChanceDisplay = null;
let critMultiplierDisplay = null;
let progressBar = null;
let progressText = null;
let levelAnnounce = null;
let gameTitle = null;
let upgradeClickBtn = null;
let upgradeHelperBtn = null;
let upgradeCritChanceBtn = null;
let upgradeCritMultBtn = null;
let upgradeHelperDmgBtn = null;
...
function updateProgressBar() {
    ...
    if (progressText) {
        const text = formatString(translations[currentLanguage].progressText, {
...
function createHelperEffect() {
    ...
    if (window.playSound) playSound('helperSound');
    // Эффект попадания
    setTimeout(() => {
...
function updateUpgradeButtons() {
    ...
    // Используем новые переменные для расчета стоимости
...
function buyClickPower() {
    ...
    if (coins >= cost) {
        ...
        updateCoins();
        updateHUD();
        if (window.playSound) playSound('upgradeSound');
        if (window.showTooltip && window.hideTooltip && window.formatString) {
...
function destroyBlock(block) {
    ...
    if (comboCount > 1) {
        ...
        if (window.playSound) playSound('comboSound');
    }
    coins += reward;
    gameMetrics.blocksDestroyed++;
    updateCoins();
    updateHUD();
    if (window.playSound) playSound('breakSound');
...
function createMovingBlock() {
    if (currentBlock && gameArea && gameArea.contains(currentBlock)) {
...
function gameOver(customMessage = null) {
    ...
    if (finalScoreDisplay) {
        if (window.applyTranslation && window.formatString) {
...
function startGame(reset = true) {
    if (reset) {
        ...
        clickPower = calculateClickPower();
        // добавляем обновление UI при загрузке сохранения
        updateHUD();
        updateProgressBar();
        updateUpgradeButtons();
    }
...
function initGame() {
    ...
    let missingElements = [];
    const elements = {
        coinsDisplay, clickPowerDisplay, critChanceDisplay, critMultiplierDisplay, 
        progressBar, progressText, levelAnnounce, gameTitle,
        upgradeClickBtn, upgradeHelperBtn, upgradeCritChanceBtn, upgradeCritMultBtn, upgradeHelperDmgBtn,
    ...
    if (window.planetBackground && typeof window.planetBackground.init === 'function') {
        window.planetBackground.init();
    }
    
    // Инициализация языка
    if (typeof window.initLanguageUI === 'function') {
        window.initLanguageUI();
    }
    
    // Обработчики событий
    if (langBtnWelcome) {
        langBtnWelcome.addEventListener('click', window.switchLanguage || function() {});
        langBtnWelcome.addEventListener('touchstart', function(e) {
            e.preventDefault();
            if (window.switchLanguage) window.switchLanguage();
        }, { passive: false });
    }
    ...
    if (upgradeClickBtn) {
        upgradeClickBtn.addEventListener('mouseenter', () => {
            if (window.showTooltip) showTooltip(translations[currentLanguage].tooltips.upgradeClick);
        });
        upgradeClickBtn.addEventListener('mouseleave', hideTooltip);
    }
    if (upgradeHelperBtn) {
        upgradeHelperBtn.addEventListener('mouseenter', () => {
            if (window.showTooltip) showTooltip(translations[currentLanguage].tooltips.upgradeHelper);
        });
        upgradeHelperBtn.addEventListener('mouseleave', hideTooltip);
    }
    if (upgradeCritChanceBtn) {
        upgradeCritChanceBtn.addEventListener('mouseenter', () => {
            if (window.showTooltip) showTooltip(translations[currentLanguage].tooltips.upgradeCritChance);
        });
        upgradeCritChanceBtn.addEventListener('mouseleave', hideTooltip);
    }
    if (upgradeCritMultBtn) {
        upgradeCritMultBtn.addEventListener('mouseenter', () => {
            if (window.showTooltip) showTooltip(translations[currentLanguage].tooltips.upgradeCritMult);
        });
        upgradeCritMultBtn.addEventListener('mouseleave', hideTooltip);
    }
    if (upgradeHelperDmgBtn) {
        upgradeHelperDmgBtn.addEventListener('mouseenter', () => {
            if (window.showTooltip) showTooltip(translations[currentLanguage].tooltips.upgradeHelperDmg);
        });
    ...
    if (saveBtn) {
        saveBtn.addEventListener('click', window.saveGame || function() {});
    ...
    if (window.updateLangButtonTooltip) window.updateLangButtonTooltip();
    // Вызов updateContinueButton после инициализации всех функций
    ...
    window.addEventListener('resize', function() {
        ...
        if (window.planetBackground && window.planetBackground.setCanvasSize) {
            window.planetBackground.setCanvasSize();
        }
    });
